...
cohort_imputed <- cohort_imputed %>%
  mutate(
    ...
    Isit_Imputed_weighted_median_ADI_STATERNK = ifelse(is.na(weighted_median_ADI_STATERNK), 1, 0),
    Isit_Imputed_weighted_mean_miles_to_UCSF = ifelse(is.na(weighted_mean_miles_to_UCSF), 1, 0),
    Isit_Imputed_weighted_mean_hours_to_UCSF = ifelse(is.na(weighted_mean_hours_to_UCSF), 1, 0)
  )
...
cohort_imputed <- cohort_imputed %>%
  ...
  group_by(PatientDurableKey) %>%
  mutate(
    BMI_before_CI = first(BMI_before_CI),
...
#cohort_imputed <- readRDS("imputed_full_data_3.rds")
...
cohort_nona <- cohort_imputed %>%
  filter(
    !is.na(imputed_RaceEthnicity) & 
      ...
      !is.na(weighted_mean_hours_to_UCSF) &
      !is.na(weighted_median_ADI_STATERNK) &
...
ggsurvplot_combine(
  ...
  conf.int = TRUE,
  risk.table = TRUE,
  xlab = "Months Since T0",
  ylab = "Risk",
  title = "Cumulative incidence curve for Different Events with 95% Confidence Interval",
...
fgdata <- finegray(Surv(tstart, tstop, status_factor) ~ PatientDurableKey + White+ # Non-white as reference
                   ...
                   id = PatientDurableKey, data = time_varying_colitis)

# Take fgdata first
str(fgdata)

fgdata$imputed_Smoking_Status_Non_Smoker <- ifelse(fgdata$imputed_Smoking_Status_Current_Smoker == 0 & fgdata$imputed_Smoking_Status_Former_Smoker == 0, 1, 0)
fgdata$imputed_Marital_Status_Single <- ifelse(fgdata$imputed_Marital_Status_Partnered == 0 & fgdata$`imputed_Marital_Status_Separated/Divorced` == 0 & fgdata$imputed_Marital_Status_Widowed == 0, 1, 0)
fgdata$normal_bmi_time_varying <- ifelse(fgdata$overweight_time_varying == 0 & fgdata$obesity_time_varying == 0, 1, 0)
fgdata$normal_bmi_baseline <- ifelse(fgdata$overweight_baseline == 0 & fgdata$obesity_baseline == 0, 1, 0 )
...
baseline_data <- filtered_data_fg1 %>%
  ...
  dplyr::select(
    ...
    imputed_Smoking_Status_Current_Smoker,imputed_Smoking_Status_Former_Smoker, imputed_Smoking_Status_Non_Smoker,
    imputed_Marital_Status_Single, imputed_Marital_Status_Partnered, `imputed_Marital_Status_Separated/Divorced`, imputed_Marital_Status_Widowed,
    BMI_before_CI_imputed,
...
final_status_data <- filtered_data_fg1 %>%
  group_by(PatientDurableKey) %>%
  order_by(PatientDurableKey, fgstart)%>%
  summarize(
    end_status = last(fgstatus),
    time_in_study_mth = last(fgstop),
    ...
    pd1_ctla4_acc_dose = last(accumulated_pd1_ctla4_doses)
  ) %>%
  ungroup()
...
result_data$sex <- relevel(result_data$sex, ref = "male")

# Define the variables
variables <- c("White", "sex", "age", "weighted_mean_hours_to_UCSF_imputed", "weighted_mean_miles_to_UCSF_imputed", "weighted_median_ADI_STATERNK_imputed", "imputed_Smoking_Status",
               "imputed_Marital_Status", "BMI_before_CI_imputed","BMI_cat", "PD1", "PDL1", "CTLA4", "" , "immunodisorder",
               "gord", "ulcer", "ibs", "other_gi_disorder", "melanoma", "other_skin_tumour", "nsclc",
               "other_lung_tumour", "rcc", "head_n_neck_cancer", "appendectomy", "depression", "bipolar",
               "other_mood_disorder", "anxiety", "schizophrenia_related_psychosis")
...
# Save the document
print(doc, target = "Baseline_Statistics_180724.docx")
